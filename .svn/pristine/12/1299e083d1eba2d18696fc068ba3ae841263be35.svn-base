...
--
-- Procedure `qpsis`.`sp_select_srv_contractor_user`
--

DROP PROCEDURE IF EXISTS `sp_select_srv_contractor_user`;
...
BEGIN
  ...
  SET @dpDeptId = p_dp_dept_id;
  SET @soaDeptId = p_soa_dept_id;
  SET @category = (SELECT service_category_group from debarment
      WHERE debarment_id = p_debarment_id);
  SET @contractor = (SELECT contractor_id from debarment
      WHERE debarment_id = p_debarment_id);
  SET @remarks = concat('Release Debarment of Cat ',@category,' contractor: ',@contractor);
  ...
  INSERT INTO audit_trail(
		...
		'Release Debarment', @contractor, 'A', SYSDATE(), @remarks
		);
  SET return_code = 1;
END $$
...
BEGIN
  ...
  SET @contractor = (SELECT contractor_id from debarment
      WHERE debarment_id = p_debarment_id);
  SET @remarks = concat('DELETE Debarment of Cat ',@category,' contractor: ',@contractor);
  ...
  INSERT INTO audit_trail(
		...
		'Delete Debarment', @contractor, 'A', SYSDATE(), @remarks
		);
  SET return_code = 1;
END $$
...
--
-- Procedure `qpsis`.`sp_select_debarment_date`
--

DROP PROCEDURE IF EXISTS `sp_select_debarment_date`;
DELIMITER $$

CREATE PROCEDURE `sp_select_debarment_date`(
IN p_service_category_group varchar(3),
IN p_contractor_id varchar(20),
IN p_date DATE
)
BEGIN
  select * from debarment
...
--
-- Procedure `qpsis`.`sp_select_debarment_by_cat`
--

DROP PROCEDURE IF EXISTS `sp_select_debarment_by_cat`;
DELIMITER $$

CREATE PROCEDURE `sp_select_debarment_by_cat`(
IN p_service_category_group VARCHAR(3),
IN p_date DATE
)
BEGIN
      SELECT deb.*, con.contractor_name FROM debarment AS deb LEFT JOIN contractor AS con
          ON deb.contractor_id=con.contractor_id 
