...
ALTER TABLE `work_assignment` ADD `contact_phone_number` VARCHAR(50);
ALTER TABLE `work_assignment` ADD `internet_mail` VARCHAR(50);
ALTER TABLE `work_assignment` ADD `last_update_date` DATETIME;
...
UPDATE `work_assignment` SET `last_update_date` = SYSDATE();
UPDATE `work_assignment` SET `service_contract_no` = 'GCIO 5/11-1-00-001';
UPDATE `work_assignment` SET `post_rank` = 'Engineer';


--
-- Procedure `qpsis`.`sp_select_work_assignment_header`
--

DROP PROCEDURE IF EXISTS `sp_select_work_assignment_header`;
DELIMITER $$

CREATE PROCEDURE `sp_select_work_assignment_header`()
BEGIN
    SELECT * FROM work_assignment WHERE service_category_group = "HDR";
END $$
...
CREATE PROCEDURE `sp_insert_work_assignment`(IN p_service_category_group varchar(3),
 ...
 IN p_contact_phone_number varchar(50),

 IN p_internet_mail varchar(50),
