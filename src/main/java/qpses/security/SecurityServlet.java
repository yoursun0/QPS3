...
public class SecurityServlet extends HttpServlet
{
            ...
            throws javax.servlet.ServletException, java.io.IOException
    {
        ...
        {            
            ...
            // get WA information
            WorkAssignmentDataBean waDB = getWorkAssignmentDataBean();
            ...
            throws javax.servlet.ServletException, java.io.IOException
    {
        try
        {
            ...
            if (!("cpar".equals(waType)) && !("acpar".equals(waType))){
                ...
                wac.setDepartmentId(secCtx.getUserSOADeptId());
                ...
                if ( ("acpar".equals(waType)) ){
                	...
                	if (wac.getStatus() == null){
	                	strValidationMsg = "There is no such Service Contract Reference number. Please retry.";
	                	validResult     = false;
	                }
                }
            ...
            if (waType.equals("cpar") || waType.equals("acpar")){
            	if (validResult){
            		...
            		if (wac.getServiceCategoryGroup()==null){
            			wac.setServiceCategoryGroup("N/A");
            			secDB.insertChallengeLog(secCtx.getUserId(), secCtx.getDPDeptId(), secCtx.getSOADeptId(), functionID, challengeQuestion[0], challengeQuestion[1], "N", "N", wac.getServiceCategoryGroup(), wac.getFilePartNo(), wac.getFileNo(), wac.getDepartmentId());
            		}else{
            			secDB.insertChallengeLog(secCtx.getUserId(), secCtx.getDPDeptId(), secCtx.getSOADeptId(), functionID, challengeQuestion[0], issueDateAnswer+"+"+closeDateAnswer, "N", "N", wac.getServiceCategoryGroup(), wac.getFilePartNo(), wac.getFileNo(), wac.getDepartmentId());
            		}                    
            		// Redirect to wrong password page
            ...
            else if (validResult || waType.equals("change_request"))
            {
                ...
                if (waType.equals("proposal")){
                	DebarDataBean debarDB = getDebarDataBean();
                	List<DebarInfo> debarments = debarDB.selectDebarmentByCat(scg,wac.getIssuedDate());  // Debar Logic: Should compare the "invitation date" with debarment period
            ...
            throws javax.servlet.ServletException, java.io.IOException
    {
        try
        {
            ...
            { // User ID exists but Wrong Password
                ...
                logDB.insertChangePasswordLog(secCtx.getUserId(),secCtx.getDPDeptId(),secCtx.getSOADeptId(),"N");
            ...
            logDB.insertChangePasswordLog(uStatus.getUserId(),uStatus.getDPDeptId(),uStatus.getSOADeptId(),"Y");
